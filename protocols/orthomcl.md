...
* SCO from groups.txt: You want to find all instances where all the species are present but there is only 1 gene from each. 
	* ```cat groups.txt | perl -ne "@t=split/\s+/$_;print if (@t==4)"| grep Tbat | grep Tdio | grep Tmar | Tcg | less```
	* ```cat groups.txt | perl -ne "@t=split/\s+/$_;print if (@t==4)"| grep Tbat | grep Tdio | grep Tmar | Tcg groups.SCO.txt```
	* ```wc -l groups.SCO.txt```
